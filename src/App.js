...
function App() {
  ...
  const [repo, setRepo] = useState("QuantumBazaar");
  const { register, handleSubmit } = useForm();

  function afterSubmit(GitInfo) {
    setCurrentPage(1);
    setUsername(GitInfo.username);
    setRepo(GitInfo.repo);
  }
    ...
    (async function fetchData() {
      try {
        ...
        const response = await fetch(
          `https://api.github.com/repos/${username}/${repo}/commits?page=${currentPage}&per_page=100`
        );
